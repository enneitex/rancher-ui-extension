...
onMounted(async () => {
  try {
    ...
    detailUrl.value  = generateVMUrl(config.namespace, config.service, config.port, ids.node.detailDashboardId);
    summaryUrl.value = generateVMUrl(config.namespace, config.service, config.port, ids.node.summaryDashboardId);
    ...
    console.error('[vm] NodeTab init error:', e);
