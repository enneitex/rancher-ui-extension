...
onMounted(async () => {
  try {
    ...
    detailUrl.value  = generateVMUrl(config.namespace, config.service, config.port, ids.workload.detailDashboardId);
    summaryUrl.value = generateVMUrl(config.namespace, config.service, config.port, ids.workload.summaryDashboardId);
    ...
    console.error('[vm] WorkloadTab init error:', e);
