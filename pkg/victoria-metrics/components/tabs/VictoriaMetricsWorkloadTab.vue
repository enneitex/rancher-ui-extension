...
// VM URLs
const WORKLOAD_VM_DETAIL_URL = ref('');
const WORKLOAD_VM_SUMMARY_URL = ref('');
...
const graphVars = computed(() => {
  ...
  return {
    ...
    workload: resource.value.metadata?.name || ''
  };
});
...
async function fetchVictoriaMetrics() {
  ...
  try {
    ...
    if (monitoringDetection.value?.victoriaMetrics) {
      const vmUrls = getVMResourceUrls('workload', clusterId, vmConfig.value);
      WORKLOAD_VM_DETAIL_URL.value = vmUrls.detail;
      WORKLOAD_VM_SUMMARY_URL.value = vmUrls.summary;
      
      showVMMetrics.value = await allVmDashboardsExist(
        store, 
        clusterId, 
        [WORKLOAD_VM_DETAIL_URL.value, WORKLOAD_VM_SUMMARY_URL.value],
        vmConfig.value.getClusterConfig(clusterId)
      );
    }
  } catch (error) {
    console.warn('Error setting up Victoria Metrics for Workload:', error);
...
<template>
  <div class="victoria-metrics-workload-tab">
    ...
    <div v-else-if="!showVMMetrics" class="no-metrics">
      <div class="text-center p-20">
        <p class="text-muted">
          Victoria Metrics dashboards not available for this workload.
        </p>
      </div>
    </div>
    ...
    <div v-else>
      <VictoriaMetricsWrapper
        :detail-url="WORKLOAD_VM_DETAIL_URL"
        :summary-url="WORKLOAD_VM_SUMMARY_URL"
        :vars="graphVars"
        graph-height="600px"
      />
    </div>
